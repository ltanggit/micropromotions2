...
type Job = {
  ...
  title: string;
  description?: string;
  link: string; // Spotify track url/uri/id
  tags?: string[];
...
export default function WorkerJobPage({ params }: { params: { jobId: string } }) {
  ...
  if (error || !job) return <div className="p-6 text-red-700">{error || 'Job not found'}</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 flex flex-col gap-6">
      ...
      {/* Review form – uses your existing POST /api/jobs/:jobId/review */}
      <ReviewForm
        jobId={job._id}
        canSubmit={unlock}
        apiBase={API_BASE}
        onSubmitted={() => {
          // e.g. toast or redirect to dashboard
        }}
      />
    </div>
  );
